...
# Generate .d dependency files so objects are rebuilt when included headers change
CFLAGS += -MMD -MP
...
# Dependency files generated by -MMD, one per object
DEPS = $(OBJS_OUTPUT:%.o=%.d)

.PHONY: all cleanobj clean qemu_start qemu_debug gdb_start
